...
│
├── [01_01]hardware-limits/   ← видео 01, часть 1: железо (отдельный проект/рендер)
...
│
└── [01_02]cache/             ← видео 01, часть 2: кэш (отдельный проект/рендер)
    └── src/scenes/
        ├── numbers.tsx       ← числа: память, throughput, задержки
        └── scaling.tsx       ← когда пора масштабироваться
```

> Каждая **часть видео — отдельный Vite-проект** в формате `[VV-PP]<имя>` (`VV` — номер
> видео, `PP` — часть): редактируется и рендерится независимо (свой редактор, свои маркеры,
> свой оверлей-клип для CapCut). Имена содержат скобки — **в шелле пути в кавычках**:
> `vite '[01_02]cache'`.
