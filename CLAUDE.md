...
covers `16:01.4–17:13.9`, `audioOffset: -961.4`. Старт сдвинут с `16:00.4` ради зазора после
`[04_11]`, конец — за секунду до «Дальше мы разберём каждый протокол». **Самая плотная часть
видео: 24 бита за 72 секунды**, и держится она только тем, что объект **один** — знакомая
пятиэтажка слева стоит от начала до конца, а каждый бит навешивает на неё свою пометку
(дверь, импульс лифта, машинерию за дверью, имя сокета, расщепление верхнего этажа).
Правая колонка сменяется трижды, и у неё **фиксированный габарит** — панель `SIDE`: без неё
ширина колонки скачет вслед за длиной строк и кадр ведёт влево-вправо на 30 единиц (строки
поэтому режутся до ~30 знаков). Кульминация — `split()`: верхняя плита не надстраивается, а
**делится внутри себя** на `ПРИКЛАДНОЙ · ПРЕДСТАВЛЕНИЯ · СЕАНСОВЫЙ`, потому что прикладной
уровень эти три уровня OSI собой закрывает, а не громоздит поверх; на `more` пометки сокета
уходят, иначе в плите становится тесно. У двери две пометки, и обе живут **в зазоре** между
плитами: имя «СОКЕТ» и «ПОТОК ИЛИ СООБЩЕНИЯ» — на самих плитах для них нет места. Провод
пиров рисуется **поверх обеих** стопок, иначе слева он уходит под плиты, а справа лежит на
них. Markers: `recap`, `agreement`, `lifts`, `door`, `full`, `via`, `orabove`, `useful`,
`below`, `yours`, `nottech`, `four`, `p1`–`p4`, `peers`, `lift2`, `headers`, `onedoor`,
`socket`, `forty`, `hides`, `promises`, `more`, `three`, `firstvideo`, `why`, `end` (71.6 s).
Run `npm run serve:sum` (or `task serve:sum`).
